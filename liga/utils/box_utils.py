...
def boxes3d_to_grid3d_kitti_camera(boxes3d, calib, image_id, size=28, bottom_center=True, surface=False):
    """
    :param boxes3d: (N, 7) [x, y, z, l, h, w, ry] in camera coords, see the definition of ry in KITTI dataset
    :param bottom_center: whether y is on the bottom center of object
    :return: corners3d: (N, 8, 3)
    ...
    R = getattr(calib,'R' + image_id)
    T = getattr(calib,'T' + image_id)
    trans = np.concatenate((R,T.reshape(3,1)),axis=1)
    corners = \
        np.concatenate((corners,np.ones((rotated_corners.shape[0],rotated_corners.shape[1],1))),axis=2)
    corners = np.matmul(corners, trans.T)
    return corners.astype(np.float32)
...
def boxes3d_kitti_camera_to_imageboxes(boxes3d, calib, image_id, image_shape=None, return_neg_z_mask=False, fix_neg_z_bug=False):
    """
    ...
    if not fix_neg_z_bug:
        ...
        corners3d = boxes3d_to_grid3d_kitti_camera(boxes3d, calib, image_id,size=7, surface=False)
        if num_boxes != 0:
            num_points = corners3d.shape[1]
            pts_img, pts_depth = calib.rect_to_img(corners3d.reshape(-1, 3), image_id)
            corners_in_image = pts_img.reshape(num_boxes, num_points, 2)
            depth_in_image = pts_depth.reshape(num_boxes, num_points)
            indices = [d.sum() > 0 for d in depth_in_image]
            if any(indices):
                corners_in_image = corners_in_image[indices]
                depth_in_image = depth_in_image[indices]
...
def boxes3d_kitti_camera_to_imagecenters(boxes3d, calib, image_id, image_shape=None):
    centers3d = boxes3d_to_corners3d_kitti_camera(boxes3d).mean(1)

    pts_img, _ = calib.rect_to_img(centers3d, image_id)
