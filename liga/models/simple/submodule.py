...
def pad3d(x):
    s = list(x.shape)
    s[-1] = 1
    x = torch.cat((x,torch.zeros(s).cuda()),dim=-1)
    s = list(x.shape)
    s[-2] = 1        
    x = torch.cat((x,torch.zeros(s).cuda()),dim=-2)  
    s = list(x.shape)
    s[-3] = 1        
    x = torch.cat((x,torch.zeros(s).cuda()),dim=-3)  
    return x
...
class hourglass(nn.Module):
    def __init__(self, inplanes, gn=False):
        ...
        self.conv5 = nn.Sequential(
            nn.ConvTranspose3d(inplanes * 2,
                               ...
                               kernel_size=3,
                               padding=1,
                               output_padding=0,   #changed by me 
                               stride=2,
                               bias=False),
        ...
        self.conv6 = nn.Sequential(
            nn.ConvTranspose3d(inplanes * 2,
                               ...
                               bias=False),
            nn.BatchNorm3d(inplanes)
            if not gn else nn.GroupNorm(16, inplanes))  # +x
    ...
    def forward(self, x, presqu, postsqu):
        ...
        if presqu is not None:
            ...
            # post = F.relu(self.conv5(out) + pre, inplace=True) #changed by me            
            post = F.relu(pad3d(self.conv5(out)) + pre, inplace=True)
        out = pad3d(self.conv6(post))  # in:1/8 out:1/4   #changed by me 
...
class feature_extraction_neck(nn.Module):
    def __init__(self, cfg):
        ...
        #     convbn(stereo_dim, self.stereo_dim[0], 1, 1, 1, gn=cfg.GN),
        #     nn.ReLU(inplace=True),
