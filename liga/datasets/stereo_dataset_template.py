...
from liga.datasets.processor.lidar_map_encoder import PointMapEncoder
...
class StereoDatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        if self.dataset_cfg.get('POINT_MAP_ENCODING', None):
            self.point_map_encoder = PointMapEncoder(
                self.dataset_cfg.POINT_MAP_ENCODING
            )
        if self.dataset_cfg.get('LIDAR_ENCODING', None):
            self.point_map_encoder = PointMapEncoder(
                self.dataset_cfg.LIDAR_ENCODING
            )
    ...
    def prepare_data(self, data_dict):
        ...
        if 'gt_boxes' in data_dict:
            ...
            # for camera in data_dict['images_id']:
            #     image_id = camera.split('_')[1]
            #     bbox = data_dict['bbox' + image_id]
            #     data_dict['gt_centers_2d' + image_id] =\
            #         np.array([(bbox[:,0] + bbox[:,2]) / 2., (bbox[:,1] + bbox[:,3]) / 2.]).T
            for image_id in data_dict['images_id']:
                image_id = str(image_id)
                data_dict['gt_boxes_2d_' + image_id], indices = box_utils.boxes3d_kitti_camera_to_imageboxes(
                        gt_boxes_no3daug_cam, data_dict['calib'], image_id, image_shape, fix_neg_z_bug=True)
                data_dict['gt_centers_2d_'+ image_id] = box_utils.boxes3d_kitti_camera_to_imagecenters(
                        gt_boxes_no3daug_cam[indices], data_dict['calib'], image_id, image_shape)

        # added by psl
        if self.point_map_encoder:
            data_dict = self.point_map_encoder.forward(data_dict)
    ...
    def collate_batch(batch_list, _unused=False):
            ...
            for key, val in cur_sample.items():
                if key == 'images_id':
                    data_dict[key] = val
                      ...
                      data_dict[key] = val
                elif key in cur_sample['images_id']:
        ...
        for key, val in data_dict.items():
            if key in ['voxels', 'lidar_voxels', 'lidar_input_features', 'voxel_num_points', 'lidar_map_features', 'lidar_map']:
                ret[key] = np.concatenate(val, axis=0)
            elif key in ['points', 'lidar_coordinates', 'voxel_coords']:
                coors = []
                for i, coor in enumerate(val):
                    coor_pad = np.pad(coor, ((0, 0), (1, 0)), mode='constant', constant_values=i)
                    coors.append(coor_pad)
                ret[key] = np.concatenate(coors, axis=0)
            elif key in ['image_' + str(image_id) for image_id in data_dict['images_id']] \
                        + ['depth_gt_img', 'depth_fgmask_img']:
            ...
            elif key in ['gt_boxes', 'gt_boxes_no3daug', 'gt_boxes_2d', \
                        'gt_centers_2d', 'gt_boxes_2d_ignored',       'gt_boxes_camera'] +\
                        ['gt_centers_2d_' + str(k) for k in data_dict['images_id']] + \
                        ['gt_boxes_2d_' + str(k) for k in data_dict['images_id']] + \
                        ['bbox' + str(k) for k in data_dict['images_id']]:
            ...
            elif key in ['calib', 'calib_ori', 'use_lead_xyz', 'images_id', 'cur_id']:
                ret[key] = val
