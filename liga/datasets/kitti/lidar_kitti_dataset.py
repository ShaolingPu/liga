...
class LiDARKittiDataset(DatasetTemplate):
    ...
    def get_image_shape(self, idx):
        img_file = self.root_split_path / 'image0' / ('%s.png' % idx)
    ...
    def get_label(self, idx):
        ...
        return object3d_kitti.get_objects_from_label(label_file,self.dataset_cfg['CAMERAS'])
    ...
    def get_infos(self, num_workers=1, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                #    annotations['bbox' + camera_id] = \
                #       np.concatenate([getattr(obj,'box2d' + camera_id).reshape(1, 4) for obj in obj_list], axis=0)
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=1, create_infos=True, create_gt_database=True):
...
if __name__ == '__main__':
    ...
    if args.command == 'create_kitti_infos':
        ...
        create_kitti_infos(
            ...
            class_names=dataset_cfg['CLASSES'].keys(),   #changed by me
            data_path=ROOT_DIR / 'data' / 'zjdata',
            save_path=ROOT_DIR / 'data' / 'zjdata',
            create_infos=True,
